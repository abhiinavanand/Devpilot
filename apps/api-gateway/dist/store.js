...
function ensureDefaultProject() {
    ...
    run(`INSERT INTO projects (id, name, project_key, description, owner, owner_email, members_json, service_name, deployment_platform, app_url, deployment_webhook_token, status, created_at, updated_at)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`, id, DEFAULT_PROJECT_NAME, 'DP', 'Core SaaS workspace for project management and DevOps observability.', 'Platform Team', '', json([]), 'api-gateway', 'Other', 'http://localhost:3000/health', (0, uuid_1.v4)(), 'Active', timestamp, timestamp);
    return id;
}
...
const projectHealthCheckFromRow = (row) => ({
    id: row.id,
    projectId: row.project_id,
    appUrl: row.app_url,
    status: row.status,
...
const ensureProjectWebhookTokens = () => {
    all(`SELECT id FROM projects WHERE deployment_webhook_token IS NULL OR deployment_webhook_token = ''`).forEach((project) => {
        run(`UPDATE projects SET deployment_webhook_token = ? WHERE id = ?`, (0, uuid_1.v4)(), project.id);
    });
};
const readStore = () => ({
...
const createProject = (input) => {
    ...
    run(`INSERT INTO projects (id, name, project_key, description, owner, owner_email, members_json, service_name, deployment_platform, app_url, deployment_webhook_token, status, created_at, updated_at)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`, project.id, project.name, project.projectKey, project.description, project.owner, project.ownerEmail, json(project.members), project.serviceName, project.deploymentPlatform, project.appUrl, project.deploymentWebhookToken, project.status, project.createdAt, project.updatedAt);
    return project;
};
...
const updateProject = (id, patch) => {
    ...
    run(`UPDATE projects SET name = ?, project_key = ?, description = ?, owner = ?, owner_email = ?, members_json = ?, service_name = ?, deployment_platform = ?, app_url = ?, deployment_webhook_token = ?, status = ?, updated_at = ? WHERE id = ?`, updated.name, updated.projectKey, updated.description, updated.owner, updated.ownerEmail, json(updated.members), updated.serviceName, updated.deploymentPlatform, updated.appUrl, updated.deploymentWebhookToken || (0, uuid_1.v4)(), updated.status, updated.updatedAt, id);
    return updated;
};
...
const createDeployment = (input) => {
    ...
    if (existing) {
        run(`UPDATE deployments SET provider = ?, service = ?, environment = ?, status = ?, version = ?, deployment_url = ?, commit_sha = ?, branch = ?, triggered_by = ?, external_id = ?, started_at = ?, duration_minutes = ? WHERE id = ?`, deployment.provider, deployment.service, deployment.environment, deployment.status, deployment.version, deployment.deploymentUrl, deployment.commitSha, deployment.branch, deployment.triggeredBy, deployment.externalId, deployment.startedAt, deployment.durationMinutes, deployment.id);
        return deployment;
    }
    run(`INSERT INTO deployments (id, project_id, provider, service, environment, status, version, deployment_url, commit_sha, branch, triggered_by, external_id, started_at, duration_minutes, success_rate)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`, deployment.id, deployment.projectId, deployment.provider, deployment.service, deployment.environment, deployment.status, deployment.version, deployment.deploymentUrl, deployment.commitSha, deployment.branch, deployment.triggeredBy, deployment.externalId, deployment.startedAt, deployment.durationMinutes, 0);
    return deployment;
};
...
const recordProjectHealthCheck = (input) => {
    ...
    run(`INSERT INTO project_health_checks (id, project_id, app_url, status, status_code, response_time_ms, checked_at, error)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?)`, check.id, check.projectId, check.appUrl, check.status, check.statusCode, check.responseTimeMs, check.checkedAt, check.error);
    run(`DELETE FROM project_health_checks
     WHERE id NOT IN (
       SELECT id FROM project_health_checks WHERE project_id = ? ORDER BY checked_at DESC LIMIT 100
     ) AND project_id = ?`, check.projectId, check.projectId);
    return check;
};
