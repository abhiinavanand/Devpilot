...
exports.pushMetricsToGrafanaCloud = void 0;
const axios_1 = __importDefault(require("axios"));
const protobufjs_1 = __importDefault(require("protobufjs"));
const snappyjs_1 = __importDefault(require("snappyjs"));
const store_1 = require("./store");
const root = protobufjs_1.default.Root.fromJSON({
    nested: {
        prometheus: {
            nested: {
                WriteRequest: {
...
const series = (name, value, timestamp, labels = {}) => ({
    labels: [
...
const collectSeries = async (timestamp) => {
...
const pushMetricsToGrafanaCloud = async (reason, options = {}) => {
    ...
    const now = Date.now();
    if (!options.force && now - lastPushAt < 60000) {
    ...
    const compressed = Buffer.from(snappyjs_1.default.compress(payload));
    await axios_1.default.post(url, compressed, {
...
exports.pushMetricsToGrafanaCloud = pushMetricsToGrafanaCloud;
